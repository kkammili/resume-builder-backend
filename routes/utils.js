...
const RESUME_FORMAT_PROMPT = path.join(
  __dirname,
  "../prompts/1.resumeFormat.json"
);
const JD_PROMPT = path.join(__dirname, "../prompts/2.jdPrompt.json");
const SKILL_CMP_PROMPT = path.join(
  __dirname,
  "../prompts/3.compareSkillsPrompt.json"
);
const PROF_SUMM_PROMPT = path.join(
  __dirname,
  "../prompts/4.professionalSummaryPrompt.json"
);
const GEN_POINTS_PROMPT = path.join(
  __dirname,
  "../prompts/5.generateResumePtPrompt.json"
);
...
export const genOldFormattedResume = async (resume) => {
  ...
  try {
    ...
    resumeFormatPrompt = resumeFormatPrompt.prompt.replace(
      "{{resume}}",
      resume
    );
...
export const processSkills = async (resumeSkills, jobDescSkills) => {
  ...
  let updatedResumeSkills = mergeSkills(
    resumeSkills,
    aiCategorizedSkills.categorized_skills
  );
...
const identifyMissingSkills = (resumeSkills, jobDescSkills) => {
  const flattenedResumeSkills = Object.values(resumeSkills)
    .flat()
    .map((skill) => skill.toLowerCase());

  return jobDescSkills.filter((skill) => {
    const lowerSkill = skill.toLowerCase();
    return !flattenedResumeSkills.some((existingSkill) =>
      existingSkill.startsWith(lowerSkill)
    );
  });
};
...
const categorizeSkillsWithAI = async (missingSkills) => {
  ...
  try {
    cmpSkillsPrompt = cmpSkillsPrompt.prompt.replace(
      "{{missingSkills}}",
...
const generateResumePoints = async (missingSkills) => {
  ...
  try {
    resPointPrompt = resPointPrompt.prompt.replace(
      "{{missingSkills}}",
 ...
 */
const updateResumeWithGeneratedPoints = (
  workExperience,
  generatedResumePoints
) => {
  console.log("🚀 Creating a new enhanced work experience...");

  // ✅ Extract actual resume points from OpenAI response
  const actualResumePoints =
    generatedResumePoints.resume_points || generatedResumePoints;
  ...
  Object.entries(actualResumePoints).forEach(([skill, resumePoints]) => {
    ...
    if (!Array.isArray(resumePoints)) {
      console.error(
        `⚠️ Skipping invalid resume points for ${skill}:`,
        resumePoints
      );
      return;
    }
    ...
    newWorkExperience.forEach((job) => {
      job.projects.forEach((project) => {
        ...
        if (
          techStackLower.some((existingSkill) =>
            existingSkill.startsWith(skill.toLowerCase())
          )
        ) {
      ...
      newWorkExperience.forEach((job, index) => {
        job.projects.forEach((project) => {
          ...
          resumePoints.forEach((point, i) => {
            if (
              !assignedPoints.includes(point) &&
              i % newWorkExperience.length === index
            ) {
              project.keyAchievements.push(point);
              assignedPoints.push(point);
...
export const genWorkExperience = async (workExperience, missingSkills) => {
  ...
  const updatedWorkExperience = updateResumeWithGeneratedPoints(
    workExperience,
    generatedResumePoints
  );
